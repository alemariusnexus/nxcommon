...
TEST(ByteArrayTest, MainByteArrayTest)
{
...

	{
		...
		CString cstr1(barr);
		EXPECT_NE((const char*) barr.get(), cstr1.get());
		EXPECT_TRUE(memcmp((const char*) barr.get(), cstr1.get(), 42) == 0);
		EXPECT_EQ('\0', cstr1[42]);

		barr = ByteArray(data, 42, 43);
		CString cstr2(barr);
		EXPECT_EQ((const char*) barr.get(), cstr2.get());
		...
		EXPECT_NE(barr.get(), (const uint8_t*) ustr1.get());
		...
		EXPECT_NE(barr.get(), (const uint8_t*) ustr2.get());
		EXPECT_TRUE(memcmp(barr.get(), ustr2.get(), 42) == 0);
		EXPECT_EQ(UTF16_LITC('\0'), ustr2[21]);

		barr = ByteArray(data, 42, 44);
		UString ustr3(barr);
		EXPECT_EQ(21, ustr3.size());
		EXPECT_EQ(barr.get(), (const uint8_t*) ustr3.get());
...

	{
		...
		dataCpy = new uint8_t[sizeof(data)];
		...
		dataCpy = (uint8_t*) malloc(sizeof(data));
		memcpy(dataCpy, data, sizeof(data));
		barr = ByteArray::fromCustomDelete(dataCpy, sizeof(data), [](uint8_t* v) { free(v); });
		EXPECT_EQ(dataCpy, barr.get());
		EXPECT_EQ(dataCpy, barr.mget());

		dataCpy = (uint8_t*) malloc(sizeof(data));
		memcpy(dataCpy, data, sizeof(data));
		barr = ByteArray::fromCustomDelete(dataCpy, sizeof(data), sizeof(data), [](uint8_t* v) { free(v); });
		EXPECT_EQ(dataCpy, barr.get());
		EXPECT_EQ(dataCpy, barr.mget());
...

	{
		...
		EXPECT_EQ(cstr2.get(), (const char*) barr2.get()); // Should be using read-aliasing instead of copy
	}

	{
		...
		ByteArray barr1;
		{
			CString cstr2;
			{
				...
				EXPECT_EQ(cstr1.get(), (const char*) barr1.get());
			}
		...
		EXPECT_EQ(cstr1Ptr, (const char*) barr1.get());
		EXPECT_TRUE(strcmp("Hello World", (const char*) barr1.get()) == 0); // Should still have cstr1's null terminator
	}
}
