...
MACRO(FIND_BUILD_COMPILER)
    IF(CMAKE_CROSSCOMPILING)
        ...
        SET(BUILD_CXX_COMPILER_ID "" CACHE STRING "Type of the C++ build compiler in BUILD_CXX_COMPILER (e.g. GNU, MSVC)")
    ELSE()
        SET(BUILD_CXX_COMPILER "${CMAKE_CXX_COMPILER}")
        SET(BUILD_CXX_COMPILER_ID "${CMAKE_CXX_COMPILER_ID}")
    ENDIF()
    
    IF(NOT EXISTS "${BUILD_CXX_COMPILER}")
        MESSAGE(SEND_ERROR "Variable BUILD_CXX_COMPILER does not point to a valid build compiler!")
    ENDIF()
    IF("z${BUILD_CXX_COMPILER_ID}" STREQUAL "z")
        MESSAGE(SEND_ERROR "Variable BUILD_CXX_COMPILER_ID is not set!")
    ENDIF()
